...
export default function VirtualizedMultiSelect({
  ...
  const visibleItems = useMemo(() => {
    ...
    const visibleCount = Math.ceil(maxHeight / itemHeight) + 5 // Buffer for smooth scrolling
  ...
  }, [filteredOptions, scrollTop, itemHeight, maxHeight, shouldVirtualize])
  ...
  const renderOption = (option: Option, index: number, style?: React.CSSProperties) => {
        ...
        <div className="absolute top-full left-0 z-50 mt-1 w-full rounded-md border bg-popover text-popover-foreground shadow-md">
          ...
          >
            {filteredOptions.length === 0 ? (
              ...
              <div style={{ height: (visibleItems as any).totalHeight, position: 'relative' }}>
                {(visibleItems as any).items.map((option: Option, index: number) => {
                  const actualIndex = (visibleItems as any).startIndex + index
          ...
          <div className="border-t p-2 text-xs text-muted-foreground text-center">
            {searchTerm && `Showing ${filteredOptions.length} of ${options.length} options`}
            {shouldVirtualize && !searchTerm && `${filteredOptions.length} options (virtualized)`}
          </div>
        </div>
      )}
    </div>
